window.addEventListener("DOMContentLoaded", () => {
  ...
  dropdownList.addEventListener("click", (event) => {
    ...
    if (!item) return; // клик не по нужному элементу

    const name = item.querySelector(".item-title").textContent; // правильный класс для имени
    selectedComputer = item.dataset.id; // dataset.id, а не dataset.value
    dropdownBtn.textContent = name;
    dropdownList.style.display = "none";
  });

  //Убрать меню если нажал за пределы списка
